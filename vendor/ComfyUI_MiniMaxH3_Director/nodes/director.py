            ...
            "optional": {
                ...
                "dual_clock": (
                    "BOOLEAN",
                    {
                        "default": False,
                        "tooltip": (
                            "【已停用】T8 双时钟采样方案已整体移除，此处固定无效。\n"
                            "采样一律走官方单时钟（稳定、可出片）。\n"
                            "⚠ 该输入槽位保留仅为兼容已保存的工作流：ComfyUI 的 widgets_values "
                            "按位置序列化，删掉这一项会让后续所有参数整体错位。"
                        ),
                    },
                ),
                "attention_accel": (
                ...
                "steps_audio": (
                    "INT",
                    {
                        ...
                        "tooltip": (
                            "【已停用】音频独立步数随 T8 双时钟方案一并移除，固定无效。\n"
                            "音频与视频共用「步数」（低于 8 步时由音频护栏自动抬升）。\n"
                            "槽位保留仅为兼容已保存工作流的位置序列化。"
                        ),
                    },
                ),
                **director_perf_inputs(),
