"""音频完整性审计（Audio Integrity Audit）——双时钟采样的"输出端"保险。

与 ``dual_clock_sampling`` 的关系
--------------------------------
双时钟解决的是**采样阶段**的音频完整性：让音频在自己的时钟上推进，
低步数（4/8 步）下音频不会因为跨度过大而被拉成白噪声。

本模块解决**输出阶段**的音频完整性：音轨解码 + 裁剪 + 拼接之后，
长度/采样率是否与画面严格对齐，是否出现爆音/直流跳变/尾接头等异常。
